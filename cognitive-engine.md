...
The AI should silently analyze these signals to determine which rules to activate:

```yaml
context_triggers:
  api_development:
    keywords: ["endpoint", "route", "API", "service", "request", "response"]
    auto_activate: [RateLimitGuard, InputSanitization, AuthBypassPrevention, IdempotencyRules]
    
  data_processing:
    keywords: ["database", "query", "data", "model", "schema"]
    auto_activate: [InputSanitization, O1_PrefRule, CachingPatterns, ErrorSurfaceReduction]
    
  user_interface:
    keywords: ["form", "input", "user", "interface", "frontend"]
    auto_activate: [InputSanitization, ErrorSurfaceReduction, SOLID]
    
  authentication:
    keywords: ["login", "auth", "password", "token", "session"]
    auto_activate: [ThreatModel, AuthBypassPrevention, InputSanitization, RateLimitGuard]
    
  payment_processing:
    keywords: ["payment", "transaction", "money", "billing", "charge"]
    auto_activate: [ThreatModel, IdempotencyRules, InputSanitization, ErrorSurfaceReduction, RetrySafety]
    
  performance_critical:
    keywords: ["scale", "performance", "optimization", "speed", "efficiency"]
    auto_activate: [O1_PrefRule, CachingPatterns, RetrySafety]
    
  legacy_refactoring:
    keywords: ["refactor", "improve", "cleanup", "legacy", "technical debt"]
    auto_activate: [DRY, SOLID, CodeCommentsGuideline, ErrorSurfaceReduction]
```

## 🎯 Rule Activation Matrix

### Silent Decision Tree
```
WHEN analyzing_task:
  IF contains_user_input THEN always_apply(InputSanitization)
  IF public_facing THEN always_apply(RateLimitGuard, AuthBypassPrevention)
  IF involves_state_change THEN always_apply(IdempotencyRules)
  IF performance_mentioned THEN always_apply(O1_PrefRule, CachingPatterns)
  IF distributed_system THEN always_apply(RetrySafety, ErrorSurfaceReduction)
  IF security_sensitive THEN always_apply(ThreatModel)
  IF code_organization THEN always_apply(SOLID, DRY)
```
